...
SYSTEM_INSTRUCTION = (
    ...
    "У тебя есть доступ к поиску в интернете (Google Search) в реальном времени. "
    "Всегда проверяй актуальную информацию в сети, если пользователь спрашивает про текущие события, факты или даты. "
...
async def create_gemini_chat():
    # Асинхронное создание чата с веб-поиском
    return await client.aio.chats.create(
...
async def reset_chat(message: types.Message):
    user_id = message.from_user.id
    user_chats[user_id] = await create_gemini_chat()
    welcome_text = (
        "Привет, я Google Gemini 3.5 Flash Lite с доступом к веб-поиску!\n\n"
